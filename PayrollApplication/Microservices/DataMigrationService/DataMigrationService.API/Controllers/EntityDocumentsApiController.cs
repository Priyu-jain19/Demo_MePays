 ...
 *  Modify:                                                                                        *
 *  Description:                                                                                   *
 *  This controller handles CRUD operations for Entity master bank details entries.                *
...
namespace DataMigrationService.API.Controllers
{
    [Route("api/")]
    [ApiController]
    public class EntityDocumentsApiController : ControllerBase
    {
        private readonly IEntityDocumentsRepository _repository;
        public EntityDocumentsApiController(IEntityDocumentsRepository repository)
        {
        ...
        ///  Change Date    :      
        /// </summary>
        [HttpGet("getentitydocumentsbylogid/{id}")]
        public async Task<IActionResult> GetEntityDocumentsByLogId(int id)
        {
            ApiResponseModel<EntityDocuments> apiResponse = new ApiResponseModel<EntityDocuments>();
            // Fetch JSON result directly from the stored procedure
            var jsonResult = await _repository.GetByLogIdAsync(DbConstants.GetEntityDocumentsById, new { Log_ID = id });
        ...
        ///  Created Date   :- 18-Dec-2024    
        /// </summary>
        [HttpPost("postentitydocuments")]
        public async Task<IActionResult> PostEntityDocuments([FromBody] EntityDocuments entityDocuments)
        {
            ApiResponseModel<EntityDocuments> apiResponse = new ApiResponseModel<EntityDocuments>();
            ...
            var jsonResult = await _repository.AddJsonAsync(DbConstants.AddEntityDocuments, entityDocuments);
        ...
        ///  Change Details :- 
        /// </summary>
        [HttpPut("updateentityDocumentstage/{id}")]
        public async Task<IActionResult> updateEntityDocumentStage(int id, [FromBody] EntityDocumentsVerified entityDocumentsVerified)
        {
            ApiResponseModel<ContractorStage> apiResponse = new ApiResponseModel<ContractorStage>();
            if (entityDocumentsVerified == null || id <= 0 || entityDocumentsVerified.EntityDocumentsValidateUDT == null || !entityDocumentsVerified.EntityDocumentsValidateUDT.Any())
            {
                apiResponse.IsSuccess = false;
            ...
            entityDocumentsVerified.Log_Id = id;
            var jsonResult = await _repository.UpdateAsync(DbConstants.UpdateEntityMaster, entityDocumentsVerified);
            if (string.IsNullOrEmpty(jsonResult))
            {
                return StatusCode((int)HttpStatusCode.InternalServerError, new ApiResponseModel<string>
                {
