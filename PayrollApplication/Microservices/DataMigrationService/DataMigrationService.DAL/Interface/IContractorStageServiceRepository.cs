...
{
    public interface IContractorStageServiceRepository 
    {
        Task<string> GetByLogIdAsync(string procedureName, object parameters);
        Task<string> AddJsonAsync(string procedureName, ContractorStage model);
        Task<string> UpdateAsync(string procedureName, ContractorStage model);
    }
}
