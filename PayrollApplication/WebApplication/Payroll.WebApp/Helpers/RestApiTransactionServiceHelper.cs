...
namespace Payroll.WebApp.Helpers
{
 ...
 string apiKey)
        ...
        public async Task<DistinctLocationViewModel> BindDistinctLocationAsync(string apiKey, int companyId, int userId)
        {
            DistinctLocationViewModel obj = new DistinctLocationViewModel();

            string distinctLocationApiUrl = $"{_apiSettings.PayrollTransactionEndpoints.GetDistinctLocationUrl}?company_ID={companyId}&user_Id={userId}";
        ...
        public async Task<IsParentComponentViewModel> BindIsParentComponentAsync(string apiKey, int companyId, int? EarnDedType,bool? IsChild)
        {
            ...
            // Build API URL with conditional query parameters
            var queryParams = new List<string> { $"selectType={selectType}" };

            if (EarnDedType.HasValue)
                queryParams.Add($"EarnDedType={EarnDedType.Value}");

            if (IsChild.HasValue)
                queryParams.Add($"IsChild={IsChild.Value.ToString().ToLower()}");

            string queryString = string.Join("&", queryParams);
            string fullApiUrl = $"{_apiSettings.PayrollTransactionEndpoints.GetAllPayComponentsUrl}/{companyId}?{queryString}";
        ...
        public async Task<List<SelectListItem>> BindEnumDisplayNameToDropdownAsync<TEnum>() where TEnum : Enum
        {
            var enumValues = Enum.GetValues(typeof(TEnum)).Cast<TEnum>();
