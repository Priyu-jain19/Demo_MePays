...
function resetPTaxSlabForm() {
    ...
    resetAndRebindDropdown('#specialDeductionMonth');
    hideTaxFields();
...
// On change of frequency hide and show special deduction
//$('#frequency').on('change', function () {
//    if ($(this).val() === '1') {
//        // Hide the Select2 container span
//        $('#specialDeductionMonth').closest('.col-lg-6').hide();
//    } else {
//        // Show it for other options
//        $('#specialDeductionMonth').closest('.col-lg-6').show();
//    }
//});
...
function toggleMonthDropdown() {
    if ($('#frequency').val() === '1') {
        // Hide the month dropdown if Yearly is selected
        $('#specialDeductionMonth').closest('.col-lg-6').show();
    } else {
        // Show it for other selections
        $('#specialDeductionMonth').closest('.col-lg-6').hide();
    }
}
$('#frequency').on('change', function () {
...
function resetAndRebindDropdown(selector) {
    ...
    switch (selector) {
        ...
        case '#gender':
            ...
            placeholder = 'All';
            break;
        case '#specialDeductionMonth':
            url = '/DropDown/FetchEffectivePayrollStartMonthDropdown';
    ...
    $.ajax({
        ...
        data: data,
        success: function (response) {
            // Use value "0" for #gender only
            const placeholderValue = (selector === '#gender') ? '0' : '';
            $el.empty().append(`<option value="${placeholderValue}">${placeholder}</option>`);
...
$(document).ready(function () {
    ...
    $('#salaryFrom, #salaryTo').on('blur', function () {
        const val = parseFloat($(this).val());
        if (!isNaN(val)) {
            $(this).val(val.toFixed(2));
        }
    ...
    function validateSalaryFields() {
        let fromVal = $('#salaryFrom').val().trim();
        let toVal = $('#salaryTo').val().trim();

        $('#salaryFrom-error').text('');
        $('#salaryTo-error').text('');

        // Rule 2: Salary From cannot start with 0 (unless it's a decimal like 0.50)
        //if (fromVal.length > 0 && /^0\d/.test(fromVal)) {
        //    $('#salaryFrom-error').text('Salary Range From cannot start with 0.');
        //}

        // Rule 1: Both fields cannot have the same value
        if (fromVal !== '' && toVal !== '' && parseFloat(fromVal) === parseFloat(toVal)) {
            $('#salaryFrom-error').text('Salary From and To cannot be the same.');
            $('#salaryTo-error').text('Salary From and To cannot be the same.');
        }
    }
    ...
    $('.btn-add-hide').on('click', function () {
        ...
        //call common function for prevent negative value
        preventNegativeInputByClass('no-negative');
    ...
    $('#professionalTaxSlabForm').on('submit', function (e) {
        ...
        if (isValid) {
            var formData = {
                ...
                Gender: $('#gender').val() ? parseInt($('#gender').val()) : 0,
...
//$('#salaryFrom').on('keydown', function (e) {
//    if (e.key === '-' || e.keyCode === 189) {
//        e.preventDefault();
//    }
//});
//$('#salaryTo').on('keydown', function (e) {
//    if (e.key === '-' || e.keyCode === 189) {
//        e.preventDefault();
//    }
//});
...
function validateForm() {
    ...
    if (!validateFormRequired(salaryTo, "#salaryTo-error", "Please enter Salary range.")) {
        isPTaxSlabValid = false;
    ...
    if (!validateFormRequired(state, "#frequency-error", "Please enter frequency.")) {
        isPTaxSlabValid = false;
    }

    // Validate PT Amount
    var ptAmount = $("#ptAmount").val();
    if (!validateFormRequired(ptAmount, "#ptAmount-error", "Please enter PT Amount.")) {
        isPTaxSlabValid = false;
    }

    // Validate Effective Date
    let effectiveDate = $("#effectiveDate").val();
    if (!validateFormRequired(effectiveDate, "#effectiveDate-error", "Please enter Effective date.")) {
        isPTaxSlabValid = false;
    }

    // Validate Effective To
    var effectiveTo = $("#effectiveTo").val();
    if (!validateFormRequired(effectiveTo, "#effectiveTo-error", "Please enter Effective to date.")) {
        isPTaxSlabValid = false;
    }
    // Validate date range
    ...
    if (age && !validateNonNegative(age, "#srCitizenAge-error", "Age cannot be negative.")) {
        ...
        var numericAge = parseInt(age, 10);

        // Minimum age rule
        if (state === "19" && numericAge < 65) {
            $("#srCitizenAge-error").text("Senior Citizen age must be at least 65 years.");
            isPTaxSlabValid = false;
        } else if (state !== "19" && numericAge < 60) {
            $("#srCitizenAge-error").text("Senior Citizen age must be at least 60 years.");
            isPTaxSlabValid = false;
        }
    }
    ...
    // CASE 1: Both cannot be the same
    if (salaryFrom > 0 && salaryFrom === salaryTo) {
        $('#salaryTo-error').text('Salary Range From and To cannot be same.');
        isPTaxSlabValid = false;
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response && response.length > 0) {
                ...
                const placeholdervalue = (selector === '#gender') ? '0' : '';
                $(selector).empty().append(`<option value="${placeholdervalue}">${placeholder}</option>`);
                response.forEach(function (item) {
                    $(selector).append(new Option(item.text, item.value));
                });
...
function loadSlabDataForEdit(data) {
    ...
    fetchAndBindDropdownForEdit('/DropDown/FetchCommonStateDropdown', '#state', 'Select State', data.state_ID, { Country_Id: 99 });
    fetchAndBindDropdownForEdit('/DropDown/FetchPayTaxFrequencyDropdown', '#frequency', 'Select Frequency', data.frequency);
    fetchAndBindDropdownForEdit('/DropDown/FetchGenderDropdown', '#gender', 'All', data.gender);
    fetchAndBindDropdownForEdit('/DropDown/FetchEffectivePayrollStartMonthDropdown', '#specialDeductionMonth', 'Select Month', data.specialDeductionMonth);
