$(document).ready(function () {
    ...
    function validatePayGradeForm() {
        ...
        if (!paygradecode) {
            ...
            isValid = false;
        } else if (/^\s/.test($("#paygradecode").val())) {
            // Check if it starts with a space BEFORE trim
            $("#paygradecode-error").text("pay grade code cannot start with a space.");
            isValid = false;
        } else {
            const error = validateTextField(paygradecode);
            if (error) {
                $('#paygradecode-error').text(error);
                isValid = false;
            }
        }
        ...
        if (!paygradename) {
            ...
            isValid = false;
        } else if (/^\s/.test($("#paygradename").val())) {
            // Check if it starts with a space BEFORE trim
            $("#paygradename-error").text("pay grade name cannot start with a space.");
            isValid = false;
        } else {
        ...
        if (!isNaN(min) && !isNaN(max)) {
            if (min > max) {
                $("#maxsalary-error").text("Maximum salary must be greater than minimum salary.");
                isValid = false;
            } else if (min === max) {
                $("#maxsalary-error").text("Salary Range From and To cannot be same");
                isValid = false;
            }
        }
        return isValid;
    ...
    $(document).on('click', '#editpayGrade', function () {
        ...
        $.ajax({
            ...
            //            showAlert("danger", response.message || "An error occurred while fetching data for edit.");
            //        } catch (e) {
