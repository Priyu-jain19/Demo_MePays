        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    // ✅ CRITICAL FIX: Proper sequence for child/parent dropdown handling
                    console.log('🔄 Loading child dropdown and setting value:', data.is_Child);
                    ...
                    loadDropdownWithSelectedValue('#isChlidDropdown', '/DropDown/FetchIsChildPayComponentDropdown', function () {
                        ...
                        console.log('👶 Setting child dropdown value:', isChildValue);
                        setSelectedValueInDropdown('#isChlidDropdown', isChildValue);

                        // ✅ CRITICAL: Wait for change event to complete before handling parent
                        setTimeout(function () {
                            console.log('⏰ Child change event completed, checking parent need...');

                            // If is_Child is true, handle parent dropdown
                            if (data.is_Child && data.parent_EarningDeduction_Id) {
                                console.log('👨‍👦 Loading parent dropdown for ID:', data.parent_EarningDeduction_Id);

                                // ✅ Load parent dropdown AFTER child is set and enabled
                                loadDropdownWithSelectedValue('#isParentDropdown', '/DropDown/FetchIsParentPaycomponentDropdown', function () {
                                    console.log('📋 Parent dropdown loaded, setting value...');

                                    // ✅ Additional delay to ensure dropdown is fully rendered
                                    setTimeout(function () {
                                        const success = setSelectedValueInDropdown('#isParentDropdown', data.parent_EarningDeduction_Id);
                                        if (success) {
                                            console.log('✅ Parent dropdown value successfully set:', data.parent_EarningDeduction_Id);
                                        } else {
                                            console.error('❌ Failed to set parent dropdown value:', data.parent_EarningDeduction_Id);
                                        }
                                    }, 150); // Increased delay for better reliability
                                });
                            } else {
                                console.log('ℹ️ No parent component needed or no parent ID provided');
                            }
                        }, 300); // Increased delay to ensure change event completes
                    });
...
function setSelectedValueInDropdown(selector, value) {
    ...
    if (!value) {
        console.warn(`⚠️ Empty value provided for ${selector}`);
        return false;
    }

    // ✅ IMPROVED: Enhanced retry mechanism with better logging
    function attemptSelection(retries = 5) { // Increased retries
        ...
        if (optionExists) {
            ...
            const actualValue = $dropdown.val();
            if (actualValue == value) {
                console.log(`🎯 Value confirmed set in ${selector}: ${actualValue}`);
                return true;
            } else {
                console.warn(`⚠️ Value set but not confirmed in ${selector}. Expected: ${value}, Actual: ${actualValue}`);
            }
        } else if (retries > 0) {
            console.log(`⏳ Value '${value}' not found in ${selector}, retrying... (${retries} attempts left)`);
            setTimeout(() => attemptSelection(retries - 1), 100); // Increased delay
            return false;
        } else {
            console.error(`❌ Value '${value}' not found in dropdown: ${selector} after multiple attempts`);
