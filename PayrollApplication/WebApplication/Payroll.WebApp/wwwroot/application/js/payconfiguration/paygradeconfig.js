$(document).ready(function () {
        ...
        $.ajax({
            ...
            error: function (error) {
                if (error.status === 401) {
                    ...
                    showAlert("danger", "An error occurred while saving the PayGrade: " + xhr.responseText);
                }
            }
    ...
    function setPayGradeForm(data) {
        ...
        fetchActiveGrade(data.payGrade_Id);
    ...
    function clearPayGradeForm() {
        $('#DistinctCompanyDropdown').empty().append('<option value="">Select Company</option>').val('');
        $('#DistictLocationDropdown').empty().append('<option value="">Select Location</option>').val('');
        $('#DistinctTradeType').empty().append('<option value="">Select Trade</option>').val('');
        $('#skillTypeDropdown').empty().append('<option value="">Select Skill Type</option>').val('');
        $('#payGradeType').empty().append('<option value="">Select Pay Grade</option>').val('');
        ...
        $(".input_error_msg").text('');
        // ✅ Re-populate company dropdown after clearing
        fetchCompanies();
    }
    ...
    function fetchActiveGrade(callback) {
        $.ajax({
...
$(document).on('click', '.btn-danger-light-icon[data-bs-target="#deletePayGradeConfig"]', function () {
    selectedButton = $(this);
});
//$('#confirmAreaDelete').on('click', function () {
$(document).on('click', '#confirmPayGradeConfigDelete', function () {
    ...
    console.log(rowData);
    $.ajax({
        url: '/PayConfiguration/DeletePayGradeMapping',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        ...
        success: function (response) {
            if (response && response.success) {
                ...
                showAlert("success", response.message);
                $('#deletePayGradeConfig').modal('hide');
                LoadPayGradeConfig(sessionCompanyId);
            } else {
