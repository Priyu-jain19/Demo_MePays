$(document).ready(function () {
    ...
    window.validateTextField = function (value) {
        ...
        if (startsWithSpecialCharRegex.test(value)) {
            return "Value must start with a letter or number.";
        }

        // Allow dash separately, so dash error message works
        const specialCharRegex = /[!@#$%^&*()_":;?/>><,]/;
        if (specialCharRegex.test(value)) {
            return "Special characters are not allowed.";
        }

        // Consecutive dashes (-- or more)
        const multipleDashRegex = /--+/;
        if (multipleDashRegex.test(value)) {
            return "Consecutive dashes are not allowed.";
        }
