...
// Restrict multiple tabs OLD ROHIIT CODE TIL 30-07-25
...
function restrictMultipleTabs(tabId) {
    ...
    if (activeTabs[tabId]) {
        ...
        $('#customTabURLRestrictedAlertMessage').text('This page is already open in another tab.');

        // Show the modal
        const modal = new bootstrap.Modal(document.getElementById('customTabURLRestricted'));
        modal.show();

        // Handle OK click — redirect to AccessDenied
        $('#customTabURLRestrictedAlertYes').off('click').on('click', function () {
            window.location.href = '/Base/UrlRestricted';
        });
