$(document).ready(function () {
    ...
    $("#addmapdepartmentButton").on("click", function () {
        ...
        setTimeout(() => {
            loadAllDepartments();
            loadFloors();
            // Optional: focus the first input (for better UX)
            $("#MapCompanyDropdown").focus();
        }, 300); // Small delay to ensure data is fetched
    });
    ...
    $("#MapCompanyDropdown").on("change", function () {
        ...
        if (companyId) {
            $("#mapcompanyDropdown-error").text("");//Added 22-08-25
            loadAddCompanyLocations(companyId);
        } else {
    ...
    $("#MapCountryDropdown").on("change", function () {
        $("#mapcountryDropdown-error").text("");//Added 22-08-25
    ...
    $("#MapStateDropdown").on("change", function () {
        $("#mapstateDropdown-error").text("");//Added 22-08-25
    ...
    $("#MapCityDropdown").on("change", function () {
        $("#mapcityDropdown-error").text("");//Added 22-08-25
        var selectedCountryId = $("#MapCountryDropdown").val();
    ...
    $("#MapLocationDropdown").on("change", function () {
        $("#maplocationDropdown-error").text("");//Added 22-08-25
        var selectedCountryId = $("#MapCountryDropdown").val();
        var selectedStateId = $("#MapStateDropdown").val();
    ...
    $("#MapAreaDropdown").on("change", function () {
        $("#mapareaDropdown-error").text("");//Added 22-08-25
        var selectedCountryId = $("#MapCountryDropdown").val();
        var selectedStateId = $("#MapStateDropdown").val();
        var selectedCityId = $("#MapCityDropdown").val();
    ...
    $("#MapDepartmentDropdown").on("change", function () { //Added 22-08-25
        $("#mapdepartmentDropdown-error").text("");
    });
        ...
        $.ajax({
            ...
            error: function (error) {
                if (error.status === 401) {
                    ...
                    showAlert("danger", "An error occurred while fetching request. ");
                }
            }
        });
    }
        ...
        $.ajax({
            ...
            error: function (error) {
                if (error.status === 401) {
                    ...
                    showAlert("danger", "An error occurred while request");
                }
            }
        });
    }
    ...
    $(document).on('click', '#resetButton', function () {
        const isEditMode = $('#departmentLocationId').val() !== '';

        if (isEditMode && originalDepartmentData) {
            // Reset all values but ensure proper loading sequence
            const data = originalDepartmentData;

            // Set Department ID
            $('#departmentLocationId').val(data.departmentLocationId);

            // Start cascading from company
            $('#MapCompanyDropdown').val(data.companyId).trigger('change');

            // Delay loading to ensure data availability
            setTimeout(() => {
                loadAddCompanyLocations(
                    data.companyId,
                    data.countryId,
                    data.stateId,
                    data.cityId,
                ...
                // Delay for dependent dropdowns to populate
                setTimeout(() => {
                    $('#MapDepartmentDropdown').val(data.departmentId).trigger('change');
                    $('#MapFloorDropdown').val(data.floorId).trigger('change');
            ...
            // Reset for Add mode
            resetMapDepartmentForm(); // This already clears form and resets dropdowns
            loadDropdowns(); // Reload dropdowns
        }
    });
