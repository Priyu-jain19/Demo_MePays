...
$(document).ready(function () {
    ...
    function validateField(fieldId, errorMessage) {
        ...
        if (!/^[A-Za-z0-9]/.test(value)) {
            field.addClass('error_input');
            errorElement.text("First character must be a letter or number, and cannot be a space.").show();
            return false;
        }
    ...
    $('#fname, #lname, #username').on('input', function () {
        ...
        // If first character is not letter or digit, remove it
        ...
        if (userId) {
            ...
            fetch('/User/EncryptId?id=' + encodeURIComponent(userId))
                ...
                .then(encryptedId => {
                    ...
                    window.location.href = "/User/UserProfile?userId=" + encodeURIComponent(encryptedId) + "&isFromMenu=true";
                })
    ...
    $('#savePermissions').on('click', function () {
        ...
        const dateValue = $("#effectiveFromDt").val();
        let effectiveFromDt = null;
        if (dateValue) {
            const [day, month, year] = dateValue.split('/');
            effectiveFromDt = new Date(`${year}-${month}-${day}`).toISOString().split('T')[0];
        }

        const roleId = $('#userRole').val();
        const companyIdVal = $('#Companies').val();
        const companyId = companyIdVal ? parseInt(companyIdVal, 10) : 0;
        const userIdVal = $('#userId').val();
        const userId = userIdVal ? parseInt(userIdVal, 10) : 0;

        const effectiveToDt = "2099-12-31";
...
function getSelectedPermissions() {
    ...
    $('.menu-checkbox:checked').each(function () {
        ...
        // Check if this menu has permissions (based on whether its div exists)
        const hasPerDtl = $(`#permissions-${menuId}`).length > 0;
        ...
        const permissions = {
            ...
            Permissions: {
                HasPerDtl: hasPerDtl,
                Add: $('#add-' + menuId).prop('checked') || false,
                View: $('#view-' + menuId).prop('checked') || false,
                Edit: $('#edit-' + menuId).prop('checked') || false,
                Delete: $('#delete-' + menuId).prop('checked') || false,
                Approve: $('#approve-' + menuId).prop('checked') || false, // in case you add approve later
                RptPrint: $('#rptprint-' + menuId).prop('checked') || false,
                RptDownload: $('#rptdownload-' + menuId).prop('checked') || false,
                DocDownload: $('#docdownload-' + menuId).prop('checked') || false,
                DocUpload: $('#docupload-' + menuId).prop('checked') || false
            }
        };
...
// Added by priyanshi deactivate user
$(document).ready(function () {
    $('.user-status-toggle').on('change', function () {
        ...
        const companyId = $toggle.data('companyid');
        const countryId = $toggle.data('countryid');

        const deactivationType = isActive ? 1 : 2; // Or define based on your business logic
        const deactivationReason = isActive ? "Activated by admin" : "Deactivated by admin";
        ...
        $.ajax({
            url: '/User/UpdateDeactivateUserRecord', // <-- Replace with your actual controller path
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify(payload),
            success: function (response) {
                if (response.success) {
                    $toggle.next('label').text(isActive ? 'Active' : 'Inactive');
                    showAlert("success", response.message);
                    setTimeout(function () {
                        // Redirect to the list page
                        window.location.href = "/User/Index";
            ...
            error: function () {
                ...
                $toggle.prop('checked', !isActive); // Revert toggle on error
            }
        });
    });
});
