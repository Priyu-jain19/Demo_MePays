...
$(document).ready(function () {
    ...
    $("#slabname").on("input", function () {
        if (/^\s/.test($(this).val())) {
            $(this).val($(this).val().trimStart());
        }
    });
    $('#taxpayable').on('input', function () {
        let value = $(this).val();
        // Remove invalid characters: anything except digits and optional one dot
        value = value.replace(/[^0-9.]/g, '');

        // Allow only one decimal point
        if ((value.match(/\./g) || []).length > 1) {
            value = value.replace(/\.+$/, ''); // remove extra dots from the end
        }
    ...
    function validateTaxSlabForm() {
        ...
        const decimalRegexnegative = /^(?!.*-)\d+(\.\d{1,2})?$/;

        if (!slabname) {
            $("#slabname-error").text("Please enter slab name.");
            isValid = false;
        } else if (/^\s/.test($("#slabname").val())) {
            // Check if it starts with a space BEFORE trim
            $("#slabname-error").text("Slab name cannot start with a space.");
            isValid = false;
        } else {
        ...
        if (!taxpayable) {
            ...
            isValid = false;
        } else if (!decimalRegexnegative.test(taxpayable) || parseFloat(taxpayable) < 0) {
            $("#taxpayable-error").text("Tax Payable must be a valid non-negative decimal number.");
            isValid = false;
        }
...
function LoadTaxSlab(companyId) {
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#activeSlabCount').text(result.activecount);
            $('#inactiveSlabCount').text(result.inactiveSlab);
