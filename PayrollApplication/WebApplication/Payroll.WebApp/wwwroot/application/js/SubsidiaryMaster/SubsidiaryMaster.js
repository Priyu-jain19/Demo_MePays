...
$(document).ready(function () {
	...
	if (pageLabel) {
		if (urlParams.has("subsidiary_Id")) {
			...
			pageLabel.textContent = "Update Subsidiary Company";
		} else {
			pageLabel.textContent = "Add Subsidiary Company";
		}
	} else {
...
$('#SubsidiaryCountry').on('change', function () {
	resetDropdowns(['#SubsidiaryState', '#SubsidiaryCity', '#SubsidiaryLocation', '#SubsidiaryArea']);
});
$('#SubsidiaryState').on('change', function () {
	resetDropdowns(['#SubsidiaryCity', '#SubsidiaryLocation', '#SubsidiaryArea']);
});
$('#SubsidiaryCity').on('change', function () {
	resetDropdowns(['#SubsidiaryLocation', '#SubsidiaryArea']);
});
$('#SubsidiaryLocation').on('change', function () {
	resetDropdowns(['#SubsidiaryArea']);
});
function resetDropdowns(dropdownIds) {
	dropdownIds.forEach(function (id) {
		$(id).val('').trigger('change'); // Reset dropdown
		$(`${id}-error`).text('');       // Clear any validation error message
	});
}
