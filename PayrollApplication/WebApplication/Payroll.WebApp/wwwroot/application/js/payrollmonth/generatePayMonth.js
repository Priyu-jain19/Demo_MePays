...
$('#btnGeneratePayroll').click(function () {
    ...
    const cycleType = $('#SelectedFinancialCycle').val();
...
function bindFinancialCycleDropdown() {
    $.ajax({
        url: '/DropDown/FetchFinancialCycleDropdown',
        type: 'GET',
        success: function (response) {
            if (response && response.length > 0) {
                const dropdownMenu = $('#dropdownMenuFinancialCycle');
                dropdownMenu.empty();
                ...
                $.each(response, function (i, item) {
                    ...
                    $('<li>').append(anchor).appendTo(dropdownMenu);
                });

                // Set default selected text to the first item's text
                $('#dropdownFinancialCycle').text(response[0].text);
                $('#dropdownFinancialCycle').attr('data-selected-id', response[0].value);  // optional for tracking
            }
        }
...
$('#dropdownMenuFinancialCycle').on('click', 'a.dropdown-item', function (e) {
    e.preventDefault();
    const selectedText = $(this).text();
    $('#dropdownFinancialCycle').text(selectedText);
});
//on Financial Cycle selection
$('#dropdownMenuFinancialCycle').on('click', 'a.dropdown-item', function (e) {
    e.preventDefault();

    const selectedText = $(this).text();
    const selectedId = $(this).attr('data-id'); // This is the cycle value (e.g., "1" for Financial)
    $('#SelectedFinancialCycle').val(selectedId);

    // Update dropdown button label
    $('#dropdownFinancialCycle').text(selectedText);
    $('#dropdownFinancialCycle').attr('data-selected-id', selectedId);

    const companyIdForMoth = $('#SelectedCompanyId').val(); // ✅ Get from hidden field

    // Now call the payroll month API
    if (selectedId == 1) {
        const backendStartDate = formatDateForBackend(null);
        fetchPayrollMonth(companyIdForMoth, backendStartDate, null);
    } else {
        fetchPayrollMonth(companyIdForMoth, null, selectedId);
    }
    
});

function fetchPayrollMonth(companyId, selectedDate, selectedCycle) {
    $.ajax({
        url: '/PayrollPeriod/GetPayMonthBySDate',
        type: 'POST',
        data: JSON.stringify({
            ...
            CycleType: selectedCycle  // Include this as needed by backend
        }),
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
        success: function (response) {
