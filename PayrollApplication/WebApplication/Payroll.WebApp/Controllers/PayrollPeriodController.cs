    ...
    public class PayrollPeriodController : SharedUtilityController
    {
        ...
        public async Task<IActionResult> GetPayMonthBySDate([FromBody] PayrollRequest request)
        {
            ...
            if (apiKey != null && request.CompanyId != null)
            {
                ...
                string apiUrl = $"{baseUrl}?id={request.CompanyId}&createdBy={SessionUserId}&CycleType={request.CycleType}";
		...
		[HttpPost]
        public async Task<IActionResult> GeneratePayrollPeriod(string PeriodType, string CompanyId, string PeriodFDate, string PeriodEDate,string CycleType)
        {
